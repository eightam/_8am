...
1. Search for `'_8am'` (inside single quotations) to capture the text domain.
2. Search for `_8am_` to capture all the function names.
3. Search for <code>&nbsp;_tk</code> (with a space before it) to capture DocBlocks.
4. Search for `_8am-` to capture prefixed handles.

OR

* Search for: `'_8am'` and replace with: `'megatherium'`
* Search for: `_8am_` and replace with: `megatherium_`
